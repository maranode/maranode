- [ ] **Model memory pressure eviction** — Architecture says the daemon evicts loaded models under memory pressure. Currently models are loaded on demand but never evicted until restart.
...
- [ ] **LDAP `ldap3` integration compile-time gate** — `ldap3` is in deps but it pulls in native OpenSSL which may conflict with `rustls`. Should be a feature flag.
